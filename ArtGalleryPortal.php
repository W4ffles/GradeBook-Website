...
echo("<button style='position:absolute;top:180;right:100;border:50;' onclick=\"location.href='ArtGallery.php'\">Home.</button><br />");
	...
	$_SESSION['logged'] )  //Check the flag variable to see if user has been logged in
{
	...
	elseif (mysql_num_rows($result))
	{
		...
		if ($password == $row[1])
		{
			...
			setcookie('firstname', $row[2], time()+60*60*24,'/');
			// Links for navigation to the other pages in the site.
			/*echo "Click <a href='ArtGallery.php'>here to go back Home</a>.<br />";
			echo "Click <a href='ArtGallerylogout.php'>here to Log Out</a>...<br />";	*/		
			echo "Hi $row[2],
				you are now logged in as '$row[0]'";
			echo "<h2 style='font-weight:bold;font-size:20;'>Current Info.</h2>";
			// Creates form fields to change artist info using the POST method.
			echo '<form action="ArtGalleryPortal.php" method="post" enctype="multipart/form-data"><pre>';
			...
			echo '<br />Select Image to Upload: <input type="file" name="filename" />' .
				'<br /><input type="submit" value="UPLOAD IMAGE" /></pre></form>';
